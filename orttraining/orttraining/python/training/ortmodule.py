...
def _get_device_str(device):
    if isinstance(device, str):
        ...
        if device.index is None:
            ...
            device = device.type + ':' + str(torch.cuda.index)
    elif device is None:
...
class ORTModule(torch.nn.Module):
    ...
    def cuda(self: T, device: Optional[Union[int, torch.device]] = None) -> T:
        ...
        if device is None and _get_device_str(self._device) != _get_default_device_str('cuda'):
            self._require_export = True
    ...
    def to(self: T, device: Optional[Union[int, torch.device]] = ...,
        ...
        if _get_device_str(self._device) != _get_device_str(device):
            self._require_export = True
    ...
    def to(self: T, tensor: torch.Tensor, non_blocking: bool = ...) -> T:
        ...
        device_str = _get_device_str(tensor.device)
